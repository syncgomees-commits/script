...
--// FIX: Aguardar biblioteca Xan carregar completamente
task.wait(0.5)
...
local function InitializeWindow()
    ...
    if not success then
        ...
        Xan.Notify({Title = "Erro", Content = "Falha ao criar interface!", Type = "Error"})
        return false
    end

    if not Window then
        warn("Janela retornou nil")
        return false
    end
        ...
        if input.KeyCode == Enum.KeyCode.E and Config.DebugEnabled then
            ...
            if result then
                ...
                local debugMessage = string.format(
                    "[HOLUB DEBUG] Action: RayCast | Target: %s | Position: %.2f, %.2f, %.2f",
                    targetObject.Name,
                    targetPosition.X,
                    targetPosition.Y,
                    targetPosition.Z
                ...
                warn("[HOLUB DEBUG] No object hit by raycast")
                print("[HOLUB DEBUG] No object hit by raycast")
            end
        end
        ...
        elseif name == "TeleBtn" then
            ...
            btn.MouseButton1Click:Connect(function()
                if Config.TargetPlr and Config.TargetPlr.Character then
                    ...
                    Xan.Notify({Title = "Teleport", Content = "No target selected!"})
                end
            end)
...
Xan.Notify({Title = "DEVS HUB", Content = "Loaded.", Icon = GameIcon, Type = "Success"})
